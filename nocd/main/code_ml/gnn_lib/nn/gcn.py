...
# from gnn_lib.utils import to_sparse_tensor
from main.code_ml.gnn_lib.utils import to_sparse_tensor

"""
Untuk import keluar.
BerpoDecoder dapat digunakan di luar class.
BerpoDecoder untuk bagian "Balanced Negative Log Likelihood" dari Model Bernoulli-Poisson (BP).
"""
__all__ = [
    'GCN',
    'GraphConvolution',
...
"""
Fungsi untuk menerapkan teknik regularisasi (dropout) untuk mengatasi data spare(jarang) atau dense(padat)
"""
...
class GraphConvolution(nn.Module):
    """
    Graph convolution layer. (Layer GCN)
...
class GCN(nn.Module):
    ...
    def forward(self, x, adj):
        for idx, gcn in enumerate(self.layers):
            ...
            x = gcn(x, adj) # Operasi Graph Convolution
            if idx != len(self.layers) - 1:
                x = F.relu(x) # Fungsi Aktivasi ReLU
